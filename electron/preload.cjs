...
function dlog() { if (DEBUG) { try { console.log('[preload]', ...arguments) } catch {} } }
...
const api = {
  notes: {
    ...
    update: (id, { title, content, reindex = true, reindex_now = false } = {}) => http('/notes/update', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id, title, content, reindex, reindex_now })
    }),
    delete: (id) => http(`/notes/delete?id=${encodeURIComponent(id)}`, { method: 'POST' }),
    search: (q, noteIds) => http(`/notes/search?q=${encodeURIComponent(q)}${(noteIds&&noteIds.length)?'&note_ids='+encodeURIComponent(noteIds.join(',')):''}`),
  ...
  tabs: {
    merge: async (_tabIds) => ({ ok: true }),
    unstack: async (_stackId) => ({ ok: true }),
    reorder: async (_sessionId, _tabId, _toPos) => ({ ok: true }),
